...
#Crea los vectores columnas que queramos utilizar
cluster1ID = c()
cluster1Genre = c()
cluster1Age = c()
cluster1Annual = c()
...
#Bucle que recorre todo el vector kmeans de cluster
for (i in kmeans$cluster){
  a = a+1

#añadimos las variables que nos interesen, en este caso solo mostraremos el grupo 1
  if (i == 1){
    
   cluster1ID = append(cluster1ID,cbind(dataset$CustomerID)[a,])
   cluster1Genre = append(cluster1Genre,cbind(dataset$Genre)[a,])
   cluster1Age = append(cluster1Age,cbind(dataset$Age)[a,])
   cluster1Annual = append(cluster1Annual,cbind(dataset$Annual.Income..k..)[a,])
   cluster1Score = append(cluster1Score,cbind(dataset$Spending.Score..1.100.)[a,])
  }
}
#Creamos una lista con las columnas que queramos
cluster1List=list(cluster1ID,cluster1Genre,cluster1Age)
#Convertimos la lista a dataframe
cluster1df=as.data.frame(cluster1List)
#Renombramos las columnas
names (cluster1df) = c("CustomerID","Genre","Age")
